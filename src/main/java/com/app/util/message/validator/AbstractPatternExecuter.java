...
public class AbstractPatternExecuter {
  ...
  public Map<String, Object> compileMessage(GrokPattern grokPattern, String messageLine, JsonObject jsonObject,
      int messageSeq, GrokPattern[] grokPatterns) throws NoMatchException, PartialMatchException {
    ...
    if (capture == null || capture.isEmpty()) {
      if (grokPattern.isOptional()) {
        messageSeq++;
        return compileMessage(grokPatterns[messageSeq], messageLine, jsonObject, messageSeq, grokPatterns);
      } else {
  ...
  private void matchString(Collection<Object> collection, StringBuilder sb) {
    for (Iterator<?> iterator = collection.iterator(); iterator.hasNext();) {
      ...
      if (next instanceof List) {
        ...
        matchString(list, sb);
        /*
         * for (Object object : list) { String type = (String) object; sb.append(StringUtils.trim(type)); }
         */
      } else {
